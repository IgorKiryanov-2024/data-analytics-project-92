...
left join employees on sales.sales_person_id = employees.employee_id
group by 1
having AVG(quantity * price) < (select AVG(quantity * price) from sales left join products on sales.product_id = products.product_id)
order by 2;
...
select
     ...
     EMPLOYEES.FIRST_NAME || ' ' || EMPLOYEES.LAST_NAME as SELLER,
     ROW_NUMBER() over (partition by CUSTOMERS.CUSTOMER_ID order by SALES.SALE_DATE) as SALE_NUMBER
from SALES
left join CUSTOMERS on SALES.CUSTOMER_ID = CUSTOMERS.CUSTOMER_ID
left join PRODUCTS on SALES.PRODUCT_ID = PRODUCTS.PRODUCT_ID
left join EMPLOYEES on SALES.SALES_PERSON_ID = EMPLOYEES.EMPLOYEE_ID
